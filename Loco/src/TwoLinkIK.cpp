...
*/
void TwoLinkIK::getIKOrientations(const Ogre::Vector3& p1, const Ogre::Vector3& p2, const Ogre::Vector3& n, const Ogre::Vector3& vParent, const Ogre::Vector3& nParent, const Ogre::Vector3& vChild, Ogre::Quaternion* qP, Ogre::Quaternion* qC){
	//modify n so that it's perpendicular to the vector (p1, p2), and still as close as possible to n
	Ogre::Vector3 line = p2-p1;
	Ogre::Vector3 temp = n.crossProduct(line);
	Ogre::Vector3 nG = line.crossProduct(temp);
	nG.normalise();

	//now compute the location of the child origin, in "global" coordinates
	Ogre::Vector3 solvedJointPosW = TwoLinkIK::solve(p1, p2, nG, vParent.length(), vChild.length());
	Ogre::Vector3 vParentG = solvedJointPosW-p1;
	Ogre::Vector3 vChildG = p2-solvedJointPosW;
	...
	*qC = Ogre::Quaternion(Ogre::Radian(childAngle), nParent);
}
